...
def bot_worker(bot, chat_id, mode, content="", target_id=None):
    while not stop_event.is_set():
        try:
            ...
            elif mode == 'sptag': text = f"[Sủa đi](tg://user?id={target_id}) {get_noise()}"
            elif mode == 'splag': text = ("LẮC ĐI CON CHÓ " * 50) + get_noise()
            elif mode == 'spdai': text = ("SỦA ĐI CON THÚ\n" * 50) + get_noise()
            elif mode == 'spchui': text = ("ĐỊT MẸ MÀY CON CHÓ " * 20) + get_noise()
